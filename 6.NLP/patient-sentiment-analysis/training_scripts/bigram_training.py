...
def main(feat_select: str, classifier: str, iters: str):
    ...
    logging.basicConfig(filename=f"bigrams/{feat_select}_{classifier}_{dt_string}.log", encoding="utf-8", level=logging.DEBUG)
    ...
    # Load data
    df = pd.read_csv("../../data/sentiment_analysis/tf_idf/labelled_convers_tf_idf_bigrams.csv", index_col=0)
    ...
    for iter in range(n_iters):
        ...
        clf = GridSearchCV(model, params, scoring='roc_auc_ovr', n_jobs=n_jobs, verbose=2, refit=True)
    ...
    # Save results
    with open(f"bigrams/{feat_select}_{classifier}_{dt_string}.pickle", "wb") as f:
